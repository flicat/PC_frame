...
(function(seajs) {
    ...
    seajs.config({
        ...
        alias: {
            ...
            // plugin
            'public': 'plugin/public.js?' + cacheName,
            'jquery': 'plugin/jquery.js?' + cacheName,
            'exbind': 'plugin/exbind.js?' + cacheName,
            'mvvm': 'plugin/mvvm.js?' + cacheName,
            'template': 'plugin/template.js?' + cacheName,
            'ajaxfileupload': 'plugin/ajaxfileupload.js?' + cacheName,
            'cache': 'plugin/cache.js?' + cacheName,
            'img_review': 'plugin/image-drop-review.js?' + cacheName,
            'calendar': 'plugin/calendar.js?' + cacheName,
            'timetable': 'plugin/timetable.js?' + cacheName,
    ...
    var seajs_use = seajs.use;

    seajs.use = function() {
        var args = [].slice.call(arguments);
        if(!preload_ready){
            // 加载基础模块
            seajs_use.call(seajs, ['jquery'], function(_$) {
                preload_ready = true;

                seajs_use.apply(seajs, args);
                seajs.use = seajs_use;
            });
        } else {
            seajs_use.apply(seajs, args);
